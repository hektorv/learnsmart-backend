...
1. Crear la Máquina Virtual (VPS)
Entra en tu consola de Oracle Cloud.
Ve a Compute -> Instances.
Haz clic en Create Instance.
Name: Ponle un nombre (ej: learnsmart-backend).
Image and Shape (¡IMPORTANTE!):
Haz clic en "Edit".
Image: Selecciona Canonical Ubuntu o Oracle Linux 8/9.
Shape: Selecciona Ampere (ARM).
Asegúrate de seleccionar la serie VM.Standard.A1.Flex.
Configura OCPUs a 4 y Memory a 24 GB.
Deberías ver una etiqueta que dice "Always Free Eligible".
Networking: Deja la configuración por defecto (creará una VCN y una Subnet pública).
Add SSH Keys:
Selecciona "Generate a key pair for me".
¡DESCARGA LA PRIVATE KEY! (.key o .pem). No podrás recuperarla después.
Haz clic en Create.
...
En la página de detalles de tu instancia, haz clic en el enlace de la Subnet (bajo "Primary VNIC").
Haz clic en la Security List (ej: Default Security List for...).
Haz clic en Add Ingress Rules.
Añade una regla para permitir el tráfico:
Source CIDR: 0.0.0.0/0 (Desde cualquier lugar).
Protocol: TCP.
Destination Port Range: 80, 443, 8762 (Gateway), 8080 (Keycloak).
Nota: Si quieres acceder a los microservicios directamente para debugar, añade también 8081-8085.
...
Clonar tu código: (Si tienes el repo privado, necesitarás generar un token de acceso personal en GitHub o subir tu clave SSH pública a GitHub).
...
docker compose -f docker-compose.prod.yml logs -f
Probar acceso: Desde tu navegador, entra a http://<TU_IP_PUBLICA>:8762/actuator/health (debería responder el Gateway).
